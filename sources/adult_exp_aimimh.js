...
class AdultExpAimimhSource extends ComicSource {
    name = "艾米漫画（H漫）"
    key = "h_10"
    version = "0.1.0"
    minAppVersion = "1.6.0"
    url = "https://www.aimimh.com"
    status = "experimental"

    get headers() { return { "User-Agent": "Mozilla/5.0 (Linux; Android 10; Mobile)", "Referer": this.url + "/" } }
    notReady() { throw "艾米漫画（H漫） adult experimental: 需要单独维护，尚未完成完整解析。" }
    search = { load: async () => this.notReady(), optionList: [] }
    explore = [{ title: "艾米漫画（H漫）", type: "multiPageComicList", load: async () => this.notReady() }]
